...
var RArmInitRot = script.bodyBones[3].getTransform().getLocalRotation();

function initHands(){
    script.bodyBones[0].getTransform().setLocalRotation(quat.fromEulerAngles(1, 0, 0));
    script.bodyBones[1].getTransform().setLocalRotation(quat.fromEulerAngles(0, 0, 0));
    script.bodyBones[2].getTransform().setLocalRotation(quat.fromEulerAngles(0, 0, 0));
    script.bodyBones[3].getTransform().setLocalRotation(quat.fromEulerAngles(1, 0, 0));
    script.bodyBones[4].getTransform().setLocalRotation(quat.fromEulerAngles(0, 0, 0));
    script.bodyBones[5].getTransform().setLocalRotation(quat.fromEulerAngles(0, 0, 0));
...
function checkTracking() {    
    ...
    if (!isFullBody && (script.bodytrack.isAttachmentPointTracking("LeftLeg") ||
        ...
        script.bodyBones[3].getTransform().setLocalRotation(RArmInitRot);
